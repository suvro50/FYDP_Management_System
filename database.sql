-- =============================================================================
--  FYDP MANAGEMENT & MATCHMAKING SYSTEM — VERSION 3.1 CHAMPION EDITION
--  Enterprise MySQL 8.0 Database Architecture
--  UIU — United International University
--  DBMS Lab Project | Industry Showcase Edition
-- =============================================================================
--
--  ╔══════════════════════════════════════════════════════════════════════════╗
--  ║  VERSION 3.1 — MESSAGING MODULE UPGRADE CHANGELOG                      ║
--  ║────────────────────────────────────────────────────────────────────────║
--  ║  [NEW]  group_chat_messages   — FYDP group chat (3 visibility modes)  ║
--  ║  [NEW]  direct_messages       — 1-to-1 DM between any two users       ║
--  ║  [MOD]  weekly_progress_reports — report_file_path column added       ║
--  ║  [NEW]  MODULE 6 section in schema (Messaging & Communication)        ║
--  ║  [NEW]  Indexes on messaging tables (sender, receiver, group+type)    ║
--  ║  [NEW]  Normalization proof for new tables in ADR                     ║
--  ╠══════════════════════════════════════════════════════════════════════════╣
--  ║  VERSION 3.0 — CHAMPION UPGRADE CHANGELOG                              ║
--  ║────────────────────────────────────────────────────────────────────────║
--  ║  [NEW]  sections          — section_code raw VARCHAR → FK (3NF fix)   ║
--  ║  [NEW]  trimesters        — target_trimester VARCHAR → FK (3NF fix)   ║
--  ║  [MOD]  project_groups    — section_id FK (was VARCHAR)               ║
--  ║  [MOD]  course_teacher_sections — section_id FK (was VARCHAR)         ║
--  ║  [MOD]  student_profiles  — trimester_id FK + CGPA CHECK(0-4)         ║
--  ║  [MOD]  pre_fydp_profiles — trimester_id FK + CGPA CHECK(0-4)         ║
--  ║  [MOD]  notifications     — reference_entity_type VARCHAR → ENUM      ║
--  ║  [MOD]  pre_fydp_group_members — member_role VARCHAR → ENUM           ║
--  ║  [MOD]  course_teacher_inbox   — v2.1 columns integrated              ║
--  ║  [MOD]  escalation_status ENUM — added 'IN_PROGRESS' value            ║
--  ║  [FIX]  trg_after_wpr_approve_escalate — v2.1 bug fixed               ║
--  ║  [NEW]  fn_skill_match_score — AI matchmaking score function          ║
--  ║  [NEW]  vw_pre_fydp_skill_match — matchmaking leaderboard view        ║
--  ║  [NEW]  vw_ct_dashboard — CT individual progress view (v2.1)          ║
--  ║  [NEW]  vw_ct_inbox_summary — CT inbox with progress (v2.1)           ║
--  ║  [NEW]  FULLTEXT indexes on project_title, group_name, description    ║
--  ╚══════════════════════════════════════════════════════════════════════════╝
--
--  Architecture : 3NF / BCNF throughout — zero raw VARCHAR lookup columns
--  Engine       : InnoDB (ACID, row-level locking, FK enforcement)
--  Charset      : utf8mb4 / utf8mb4_unicode_ci
--  Total Tables : 31  |  Triggers : 7  |  Procedures : 3
--  Functions    : 4   |  Views    : 9  |  Indexes    : 45+
-- =============================================================================
...
/*
╔══════════════════════════════════════════════════════════════════════════════╗
║  FYDP MANAGEMENT SYSTEM v3.1 — CHAMPION EDITION                            ║
║  ARCHITECTURE DECISION RECORD                                               ║
╠══════════════════════════════════════════════════════════════════════════════╣
║                                                                            ║
║  TOTAL: 31 Tables | 7 Triggers | 3 Procedures | 4 Functions | 9 Views     ║
║         45+ Indexes (including 2 FULLTEXT)                                 ║
║                                                                            ║
║  NORMALIZATION PROOF — ALL 31 TABLES (v3.1 additions highlighted)         ║
║  ────────────────────────────────────────────────────────────────────────  ║
║  1NF: All attributes atomic. No JSON, no repeating groups, no arrays.     ║
║  2NF: All composite-PK tables — every non-key attr depends on FULL PK.    ║
║  3NF: Zero transitive dependencies in any table.                           ║
║  BCNF: All bridge/reference tables — determiners are candidate keys.       ║
║                                                                            ║
║  [NEW v3.1] group_chat_messages — BCNF                                    ║
║    PK: {message_id} — sole candidate key and only determinant.            ║
║    FDs: message_id → group_id, sender_id, message_text,                   ║
║         attachment_path, attachment_name, chat_type, created_at           ║
║    1NF: All attributes atomic. attachment_path + attachment_name are two   ║
║         separate scalars (single attachment per message — business rule).  ║
║    2NF: Single-column PK eliminates partial dependency by definition.      ║
║    3NF: group_id ↛ sender_id, chat_type ↛ group_id, no transitive FD.    ║
║    BCNF: Only determinant is {message_id} = candidate key. ✓              ║
║    Why chat_type is IN this table and not a FK lookup:                    ║
║      chat_type is an immutable property set at write time. It does NOT    ║
║      change post-insert and has no own attributes. Normalising it into a  ║
║      separate table would add a join with zero benefit to redundancy.      ║
║      ENUM is the 1NF-correct choice for a fixed, bounded domain.          ║
║    Why attachment columns are NOT extracted to a separate table:           ║
║      Business rule = max 1 attachment per message. Two atomic scalars      ║
║      (path + name) is 1NF compliant. If multi-attachment is required      ║
║      in future, create group_chat_message_attachments (message_id, attach ║
║      pattern bridge table).                                               ║
║                                                                            ║
║  [NEW v3.1] direct_messages — BCNF                                        ║
║    PK: {message_id} — sole candidate key and only determinant.            ║
║    FDs: message_id → sender_id, receiver_id, message_text,                ║
║         attachment_path, attachment_name, is_read, created_at             ║
║    1NF: All attributes atomic. No repeating groups.                        ║
║    2NF: Single-column PK — partial dependency impossible.                  ║
║    3NF: sender_id ↛ receiver_id, is_read ↛ any other non-key attr.       ║
║    BCNF: Only determinant is {message_id} = candidate key. ✓              ║
║    Why no conversations / threads table:                                   ║
║      Conversation membership is fully derivable from (sender_id,          ║
║      receiver_id) without data loss — LEAST()/GREATEST() canonical key    ║
║      in vw_dm_conversation_threads. A conversations table would be a      ║
║      performance optimisation (pagination), not a normalisation fix.       ║
║      Can be added as a future materialised view / caching layer.           ║
║                                                                            ║
║  [NEW v3.1] report_file_path column in weekly_progress_reports            ║
║    message_id → report_file_path (NULL allowed — optional upload).        ║
║    Adding one atomic scalar to an existing BCNF table preserves BCNF.    ║
║    No new FD is introduced: file_path is fully determined by report_id.   ║
║                                                                            ║
║  v3.1 INDEXING DECISIONS — MESSAGING TABLES                               ║
║    idx_gcm_group_type   (group_id, chat_type) — primary feed query        ║
║    idx_gcm_group_created(group_id, created_at) — pagination               ║
║    idx_gcm_sender       (sender_id)            — audit / sent-messages     ║
║    idx_dm_receiver_read (receiver_id, is_read) — unread badge count       ║
║    idx_dm_sender        (sender_id)            — conversation history      ║
║    idx_dm_conversation  (sender_id, receiver_id, created_at)              ║
║                        — thread load without full table scan               ║
║                                                                            ║
║  ENGINE: InnoDB — ACID, row-level locking, FK enforcement, crash recovery ║
║  TRIGGER ORDER: Escalation fires before Audit (creation order, same event)║
║  IDEMPOTENCY: INSERT ... ON DUPLICATE KEY UPDATE on course_teacher_inbox  ║
║  SOFT-DELETE: is_active + deleted_at on users, project_groups             ║
║  CURRENT + HISTORY: users.account_status (fast O(1)) + user_status_log   ║
║                                                                            ║
╚══════════════════════════════════════════════════════════════════════════════╝
*/